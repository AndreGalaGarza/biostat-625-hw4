...
We use this helper function to verify that the calculated distance matrices are identical:

```{r}
compare_dist_calcs <- function(mat, method, diag = FALSE, upper = FALSE,
                               p = 2, display = FALSE) {
  dist_base <- stats::dist(mat, method, diag, upper, p)
  dist_reDist <- re_dist(mat, method, p)
...
```{r}
compare_dist_calcs(mat, "minkowski", p = 3, display = TRUE)
```
...
dist_reDist <- re_dist(mat, method = "euclidean")
print(dist_reDist, diag = TRUE)
```
...
dist_reDist <- re_dist(mat, method = "euclidean")
print(dist_reDist, diag = TRUE, upper = TRUE)
```
